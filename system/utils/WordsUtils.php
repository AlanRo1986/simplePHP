 ...
 *
 * example:
 * $w = app(WordsUtils::class);
 * $w->getWords($a); return dict string.
 *
 */
...
class WordsUtils extends CompactUtils
{

    //hash算法选项
    protected $mask_value = 0xFFFF;

    //输入和输出的字符编码（只允许 utf-8、gbk/gb2312/gb18030、big5 三种类型）
    protected $sourceCharSet = 'utf-8';
    protected $targetCharSet = 'utf-8';

    //生成的分词结果数据类型 1 为全部， 2为 词典词汇及单个中日韩简繁字符及英文， 3 为词典词汇及英文
    protected $resultType = 1;

    //句子长度小于这个数值时不拆分，notSplitLen = n(个汉字) * 2 + 1
    protected $notSplitLen = 5;

    //把英文单词全部转小写
    protected $toLower = false;

    //使用最大切分模式对二元词进行消岐
    protected $differMax = false;

    //尝试合并单字
    protected $unitWord = true;

    //使用热门词优先模式进行消岐
    protected $differFreq = false;

    //被转换为unicode的源字符串
    protected $sourceString = '';

    //附加词典
    protected $addonDic = array();
    protected $addonDicFile = 'dict/words_addons.dic';

    //主词典
    protected $dicStr = '';
    protected $mainDic = array();
    protected $mainDicHand = false;
    protected $mainDicInfos = array();
    protected $mainDicFile = 'dict/base_dic_full.dic';

    //是否直接载入词典（选是载入速度较慢，但解析较快；选否载入较快，但解析较慢，需要时才会载入特定的词条）
    protected $isLoadAll = false;

    //主词典词语最大长度 x / 2
    protected $dicWordMax = 14;
    //粗分后的数组（通常是截取句子等用途）
    protected $simpleResult = array();

    //最终结果(用空格分开的词汇列表)
    protected $finallyResult;

    //是否已经载入词典
    protected $isLoadDic = false;

    //系统识别或合并的新词
    protected $newWords = array();
    protected $foundWordStr = '';

    //词库载入时间
    protected $loadTime = 0.0;
    ...
    /**
     * @param string $source 一丢丢文字
     * @return string
     */
    public function getWords(string $source = ""){
        $this->SetSource($source);
        $this->StartAnalysis();
        return $this->GetFinallyKeywords();
    }
     ...
     * @return short int|bool
     */
    protected function GetWordInfos( $key, $type='word' )
    {
        if( !$this->mainDicHand )
        {
     ...
     * @throws \Exception
     */
    public function SetSource(string $source ="", $source_charset = 'utf-8', $target_charset = 'utf-8' )
    {
        ...
        if( $source != '' ) {
            if( preg_match("/^utf/", $source_charset) ) {
                ...
                throw new \Exception("Parse the source string failed. ");
            }
        } else {
            throw new \Exception("the source can not be null or ''");
        }
        return $this;
    }
     ...
     * @return $this
     */
    public function LoadDict(string $mainDic='' )
    {
        ...
        $this->loadTime = sprintf("%1\$.5f", microtime(true) - $starTime);
        $this->isLoadDic = true;
    ...
    {
        $fos = $this->GetWordInfos( $word );
        return ($fos !== false);
    }

    /**
     * 获得某个词的词性及词频信息
     * @param string $word unicode编码的词
     * @return string
     */
    public function GetWordProperty(string  $word)
    {
        if( strlen($word)<4 )
        {
            return '/s';
        }
        $info = $this->GetWordInfos($word);
        return isset($info[1]) ? "/{$info[1]}{$info[0]}" : "/s";
    }
     ...
     * 指定某词的词性信息（通常是新词）
     * @param string $word unicode编码的词
     * @param $info array('c' => 词频, 'm' => 词性);
     * @return $this
     */
    public function SetWordInfos(string $word, $info)
    {
        if( strlen($word)<4 )
        {
            return $this;
     ...
     * @return string
     */
    public function GetFinallyKeywords(int $num = 10 )
    {
        $n = 0;
     ...
     */
    public function MakeDict(string $source_file, string $target_file='' )
    {
     ...
     * 导出词典的词条
     *
     * @param string $targetFile 保存位置
     * @return bool
     */
    public function ExportDict(string $targetFile)
    {
        if( !$this->mainDicHand )
        {
            $this->mainDicHand = fopen($this->mainDicFile, 'r');
