...
class VerifyProvider extends Provider {

	protected $seKey     = ''; //验证码加密密钥
    protected $expire    = 300;     // 验证码过期时间（s）
    protected $useZh     = false;    // 使用中文验证码
    protected $useImgBg  = false;     // 使用背景图片
    protected $fontSize  = 36;     // 验证码字体大小(px)
    protected $useCurve  = true;   // 是否画混淆曲线
    protected $useNoise  = true;   // 是否添加杂点
    protected $imageH    = 0;        // 验证码图片宽
    protected $imageL    = 0;        // 验证码图片长
    protected $length    = 5;        // 验证码位数
    protected $fontttf   = '';       // 验证码字体，不设置随机获取
    protected $bg        = array(243, 251, 254);  // 背景
    ...
    public function register()
    {
        ...
        $this->setSeKey(conf("app","authToken"));
    }

    /**
     * @param string $seKey
     */
    protected function setSeKey(string $seKey)
    {
        $this->seKey = $seKey;
    }
     ...
     * @return string
     */
    protected function getSeKey(): string
    {
        return $this->seKey;
    }
	...
	public function check(string $code, string $id = '') {
		...
		// 验证码不能为空
		$session = SessionProvider::get($key);
		...
		// session 过期
		if(TIME_UTC - $secode['time'] > $this->expire) {
            SessionProvider::set($key, null);
			return false;
		}
	...
	public function entry(string $id = '') {
		...
		if($id) {
			...
			$session[$id]['time'] = TIME_UTC;  // 验证码创建时间
		} else {
			$session['code'] = $code; // 把校验码保存到session
			$session['time'] = TIME_UTC;  // 验证码创建时间
		}
        SessionProvider::set($key, $session);
